...
public class QQService {
     ...
     */
    private static final String XIAO_V_INTRO = "你好，我是省省，我是你的省钱助手，有什么问题请问我吧，我可有很多秘诀哦";

    /**
     * XiaoV listener self intro.
     */
    private static final String XIAO_V_LISTENER_INTRO = "你好，我是省省的的守护天使，请多指教！";
...
//要可以私聊省省修改这里
                            ...
                            sendToPushQQGroups(msg);                              //向群推送管理员发来的消息
        ...
        if (MSG_ACK_ENABLED) { // 如果启用了消息送达确认
            ...
            xiaoVListener = new SmartQQClient(new MessageCallback() {
                ...
                public void onGroupMessage(final GroupMessage message) {
                    ...
                    if (GROUP_SENT_MSGS.contains(content)) { // indicates message received   删除确认消息
    ...
    private void sendToForum(final String msg, final String user) {
        boolean pingbi = true;  //暂时不发论坛
        if(pingbi)return;

        final String forumAPI = XiaoVs.getString("forum.api");    //论坛访问的api
            ...
            if (StringUtils.equals(pushGroupsConf, "*")) {
                ...
                for (final Map.Entry<Long, Group> entry : QQ_GROUPS.entrySet()) {
                    ...
                    try {
                        ...
                        final GroupInfo groupInfo = xiaoV.getGroupInfo(group.getCode());   //从这里获取群里用户
                        userCount = groupInfo.getUsers().size();   //群人数
    ...
    public void onQQGroupMessage(final GroupMessage message) {
        ...
        if (StringUtils.isNotBlank(qqMsg)) {
            qqMsg = "<p>" + qqMsg + "</p>";
            sendToForum(qqMsg, userName);     //发论坛
        }
...
//接收到的qq消息
    ...
    private String answer(final String content, final String userName) {
        String keyword = "";
        String[] keywords = StringUtils.split(XiaoVs.getString("bot.follow.keywords"), ","); //keywords是为了替换模板
